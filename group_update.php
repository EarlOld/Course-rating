...
<head>
  ...
  $('input#enter').click(function() {
    ...
    var head = {head: $("input[name=head]").val()}
    var kurator = {kurator: $("input[name=kurator]").val()}
    console.log(group, name, head, kurator);
    $.ajax({
      url: 'vidd-updateGroup.php',
      type: 'GET',
      data: {group, name, head, kurator},
      success: function(data){
        //data = jQuery.parseJSON(data);
        console.log(data);
      }
  ...
  $("select[name=group]").bind("change",function(){
    ...
    $("input[name=head]").empty();
    $("input[name=kurator]").empty();
    var group = {group: $("select[name=group]").val()};
    console.log(group);
    $.ajax({
      url: 'group_updatefind.php',
      type: 'GET',
      data: group,
      success: function(data){
        data = jQuery.parseJSON(data);
        console.log(data);
        for (id in data) {
        $("input[name=name]").val(data[id].Gr_name);
          $("input[name=head]").val(data[id].Gr_head);
            $("input[name=kurator]").val(data[id].Gr_kurator);
...
<table id="info-group">
  ...
  <tr>
    <td>Староста:</td>
    <td><input id="head" type="text" name="head"></td>
  </tr>
  <tr>
    <td>Куратор:</td>
    <td><input id="kurator" type="text" name="kurator"></td>
  </tr>
  <tr>
